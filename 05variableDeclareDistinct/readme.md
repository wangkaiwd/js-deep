...
**全局变量对象`VO(G)`中**声明的用`var`声明的变量，也会给全局对象`window`中设置对应的属性。
...
即对象对应的堆中的内容(如其中存储的键值对)可以改变，变量重新赋值会使其指向的堆的地址发生变化，就会导致变量指向一个新的对象，而`const`并不允许我们指向新的对象。
...
通过文章的描述，在文末总结下`let/const/var`的别：
